...
for(i in 1:dim(nms_df)[2]){
  ...
  print(paste0(((i / dim(nms_df)[2]) * 100), "%"))
        ...
        tmp_count[, paste(names(nms_df)[i], "_two", sep="")] <- my.f2cnt(th2 = tmp, 
...
for(i in 1:dim(nms_df)[2]){
  ...
  tmp_count[, paste(names(nms_df)[i], "_three", sep="")] <- my.f3cnt(th2 = tmp, 
...
write.csv(submission, "12312015_2.csv", row.names = FALSE)
